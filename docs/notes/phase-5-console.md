...
| `WS /api/v1/apps/{id}/console/ws` | ticket-only | n/a | no session/cookie required, the ticket is the auth |
...
| `WS /api/v1/hosts/{id}/shell/ws` | ticket-only | n/a | node shell PTY |
| `POST /api/v1/vms/{id}/console/tickets` | operator | `vms.console` | mints a `vm_vnc` ticket, audits `console.open` (target `vm`) |
| `WS /api/v1/vms/{id}/vnc/ws` | ticket-only | n/a | raw RFB byte relay, no PtyBridge translation |
| `PATCH /api/v1/hosts/{id}` (extended) | admin | n/a | `node_shell_enabled` field; now audits `host.node_shell_toggle` (Task 6 fix round) |
